...
class Start extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if (!is_string($src)) {
            throw new Exception('src is invalid');
        }
        if (!is_file($src)) {
        ...
        $srcFd = fopen($src, 'r');
        if (false === $srcFd) {
            throw new Exception("Cannot open '{$src}' for read");
        }

        $dstFd = fopen($dst, $purge ? 'w' : 'a');
        if (false === $dstFd) {
            throw new Exception("Cannot open '{$dst}' for write");
        }
        ...
        while (($line = stream_get_line($srcFd, 10 * 1024, PHP_EOL)) !== false) {
            ...
            if (null !== $lastTimestamp and $time > $lastTimestamp) {
